#!/usr/bin/env bash
# infrastructure/telos_runner_spot_bootstrap.sh — provision an EC2-spot box,
# register it as an EPHEMERAL GitHub Actions self-hosted runner for
# nousergon/telos, let it pick up exactly ONE queued job, then
...
#
# Invoked by the `telos-runner-dispatcher` Lambda over SSM. That
...
#   infrastructure/telos_runner_spot_bootstrap.sh --job-id <workflow_job.id>
#
# REQUIRED SSM: /telos/runner/github_pat — fine-grained PAT,
#   owner=nousergon, repo=telos, Administration: Read and write
...
# REQUIRED AWS: instance profile telos-runner-executor-profile
#   (infrastructure/iam/telos-runner-executor-role-{trust,policy}.json) —
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --job-id) TELOS_RUNNER_JOB_ID="$2"; shift 2 ;;
    *) shift ;;
  esac
done
...
log() { echo "[telos-runner-bootstrap] $*"; }
...
TELOS_RUNNER_USER="${TELOS_RUNNER_USER:-ec2-user}"
RUN_USER_HOME="$(getent passwd "$TELOS_RUNNER_USER" | cut -d: -f6)"
[ -n "$RUN_USER_HOME" ] || RUN_USER_HOME="/home/${TELOS_RUNNER_USER}"
...
systemd-run --on-active="${MAX_RUNTIME_SECONDS}" --unit=telos-runner-watchdog \
  --description='telos-runner spot hard-timeout' /sbin/shutdown -h now \
  >/dev/null 2>&1 || log "WARN: watchdog arm failed (shutdown_behavior=terminate still applies)"
TELOS_RUNNER_SHUTDOWN_DELAY_SECONDS="${TELOS_RUNNER_SHUTDOWN_DELAY_SECONDS:-30}"
finish() {
  rc=$?
  log "exit rc=$rc — scheduling shutdown ${TELOS_RUNNER_SHUTDOWN_DELAY_SECONDS}s from now (non-blocking — this script exits immediately so the SSM agent can report the command's real status before the deferred shutdown fires, config#1472)"
  systemd-run --on-active="${TELOS_RUNNER_SHUTDOWN_DELAY_SECONDS}" --unit=telos-runner-delayed-shutdown \
    --description='telos-runner delayed self-terminate (post-SSM-report)' /sbin/shutdown -h now \
    >/dev/null 2>&1 || {
...
# ── Secrets from SSM (instance profile: telos-runner-executor-role,
# see infrastructure/iam/telos-runner-executor-role-{trust,policy}.json — a
# DEDICATED role scoped to exactly one param; see this script's header for why
# it is not a reuse of SATURDAY_SF_WATCH_PAT) ─────────────────────────────────
GH_TOKEN="$(aws ssm get-parameter --name /telos/runner/github_pat --with-decryption \
  --query 'Parameter.Value' --output text --region "$REGION" 2>/dev/null)"
if [ -z "${GH_TOKEN:-}" ]; then
  log "FATAL: GH_TOKEN missing from SSM (/telos/runner/github_pat)"
  exit 1
fi
...
JIT_CONFIG="$(curl -sS -X POST \
  ...
  "https://api.github.com/repos/nousergon/telos/actions/runners/generate-jitconfig" \
  -d "$(jq -n --arg name "$RUNNER_NAME" \
    '{name: $name, runner_group_id: 1, labels: ["self-hosted","telos-spot"], work_folder: "_work"}')" \
  | jq -r '.encoded_jit_config // empty')"
if [ -z "${JIT_CONFIG:-}" ]; then
  log "FATAL: could not mint a JIT runner config — check GH_TOKEN's Administration:write scope on nousergon/telos"
  exit 1
fi
...
log "starting run.sh with JIT config (config#2653) — name=${RUNNER_NAME} job_id=${TELOS_RUNNER_JOB_ID:-n/a}; no separate config.sh step (--jitconfig writes the settings files directly and implies --ephemeral)"
